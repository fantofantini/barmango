...
Token-based auth (works on Railway without cookie issues)
"""
...
def require_auth(f):
    @wraps(f)
    def dec(*a,**kw):
        if not get_current_user(): return jsonify({'error':'No autorizado'}),401
        return f(*a,**kw)
    return dec
...
def make_user_resp(u, token=None):
    d = {'id':u['id'],'nombre':u['nombre'],'username':u['username'],
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS drivers (
        ...
        city TEXT, state TEXT, zip TEXT, notes TEXT, photo TEXT,
        created_at TEXT DEFAULT (datetime('now'))
    );
    ...
    CREATE TABLE IF NOT EXISTS jobs (
        ...
        costo_servicio REAL DEFAULT 0, ubicacion TEXT DEFAULT '',
        notes TEXT,
        created_at TEXT DEFAULT (datetime('now')),
...
@require_auth
def get_roles():
    conn=get_db(); rows=rows_to_list(conn.execute("SELECT * FROM roles ORDER BY nombre").fetchall()); conn.close(); return jsonify(rows)

@app.route('/api/roles', methods=['POST'])
@require_admin
def create_role():
    d=request.json; conn=get_db()
    cur=conn.execute("INSERT INTO roles (nombre,descripcion,permisos) VALUES (?,?,?)",(d['nombre'],d.get('descripcion',''),json.dumps(d.get('permisos',{}))))
...
def update_role(rid):
    d=request.json; conn=get_db()
    conn.execute("UPDATE roles SET nombre=?,descripcion=?,permisos=? WHERE id=?",(d['nombre'],d.get('descripcion',''),json.dumps(d.get('permisos',{})),rid))
    conn.commit(); conn.close(); return jsonify({'message':'Actualizado'})

@app.route('/api/roles/<int:rid>', methods=['DELETE'])
@require_admin
def delete_role(rid):
    conn=get_db(); conn.execute("DELETE FROM roles WHERE id=?",(rid,)); conn.commit(); conn.close(); return jsonify({'message':'Eliminado'})
...
def get_grupos():
    ...
    rows=rows_to_list(conn.execute("""SELECT g.*,r.nombre as rol_nombre,COUNT(u.id) as usuario_count
        FROM grupos g LEFT JOIN roles r ON g.rol_id=r.id LEFT JOIN usuarios u ON u.grupo_id=g.id
...
def create_grupo():
    d=request.json; conn=get_db()
    cur=conn.execute("INSERT INTO grupos (nombre,descripcion,rol_id) VALUES (?,?,?)",(d['nombre'],d.get('descripcion',''),d.get('rol_id')))
    conn.commit(); conn.close(); return jsonify({'id':cur.lastrowid}),201
...
def update_grupo(gid):
    d=request.json; conn=get_db()
    conn.execute("UPDATE grupos SET nombre=?,descripcion=?,rol_id=? WHERE id=?",(d['nombre'],d.get('descripcion',''),d.get('rol_id'),gid))
    conn.commit(); conn.close(); return jsonify({'message':'Actualizado'})

@app.route('/api/grupos/<int:gid>', methods=['DELETE'])
@require_admin
def delete_grupo(gid):
    conn=get_db(); conn.execute("DELETE FROM grupos WHERE id=?",(gid,)); conn.commit(); conn.close(); return jsonify({'message':'Eliminado'})
...
def delete_usuario(uid):
    u = get_current_user()
    if u and u['id'] == uid: return jsonify({'error':'No puedes eliminar tu propio usuario'}),400
    conn=get_db(); conn.execute("DELETE FROM usuarios WHERE id=?",(uid,)); conn.commit(); conn.close(); return jsonify({'message':'Eliminado'})
...
def get_job(jid):
    ...
    conn.close(); return jsonify(row_to_dict(r)) if r else (jsonify({'error':'Not found'}),404)
